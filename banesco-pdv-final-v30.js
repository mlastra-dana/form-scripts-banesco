(function () {
  ...
  function injectCSS() {
    if (document.getElementById("banesco-pdv-final-v25-css")) return;

    var style = document.createElement("style");
    style.id = "banesco-pdv-final-v25-css";
    ...
    style.innerHTML = `
      ...
      /* ANIQUILACIÓN TOTAL DE LA BARRA GRIS DE DANA/EXTJS */
      .x-panel-header,
      .x-window-header,
      .x-header,
      .x-toolbar,
      ...
      /* Limpieza de fondos grises en contenedores base */
      .x-panel,
      .x-panel-body,
      .x-form,
      .x-container,
      ...
      /* ETIQUETAS DE TEXTO (Para que luzcan como placeholders) */
      ...
      .x-form-required-field {
        display: none !important; /* Quitamos asteriscos rojos */
      }

      /* ESTILO MATERIAL DESIGN PARA INPUTS (Línea inferior, sin caja) */
      ...
      .banesco-field .x-form-focus {
        border-bottom: 2px solid #007a63 !important;
      }

      /* Eliminar estilos de ExtJS en los contenedores de los inputs */
      ...
      /* BOTÓN ENVIAR VERDE Y REDONDEADO */
      .banesco-submit-wrap {
        order: 100 !important;
        grid-column: 1 / -1 !important;
      ...
      @media screen and (max-width: 768px) {
        ...
        .banesco-field, .banesco-field-empresa, .banesco-field-actividad, .banesco-field-nacionalidad, .banesco-field-documento, .banesco-field-correo, .banesco-field-telefono, .banesco-field-estado {
          grid-column: 1 / -1 !important;
        }
      }
    `;
  ...
  // Bucle agresivo para destruir estilos inyectados por ExtJS después de la carga
  function enforceStylesAggressively() {
    setInterval(function() {
      // Forzar color verde en título
      ...
      // Forzar botón verde
      var btns = document.querySelectorAll('.banesco-submit-wrap .x-btn, .banesco-submit-wrap button, .banesco-submit-wrap input[type="submit"]');
      btns.forEach(function(b) {
        b.style.setProperty('background', '#007a63', 'important');
        b.style.setProperty('background-color', '#007a63', 'important');
        b.style.setProperty('border-color', '#007a63', 'important');
        b.style.setProperty('border-radius', '30px', 'important');
      });

      // Ocultar cabeceras y fondos oscuros de DANA
      var headers = document.querySelectorAll('.x-panel-header, .x-window-header, .x-docked-top');
      headers.forEach(function(h) {
        h.style.setProperty('display', 'none', 'important');
      });

      var bodyWrappers = document.querySelectorAll('.x-panel-body, .x-window-body');
      bodyWrappers.forEach(function(bw) {
        bw.style.setProperty('background', 'transparent', 'important');
      });
    }, 500); // Se ejecuta cada medio segundo para ganarle a DANA
  }
  ...
  function apply() {
    ...
    if (ok) {
      enforceStylesAggressively();
      console.log("Banesco PDV final v25 aplicado correctamente con forzado de estilos.");
    }
