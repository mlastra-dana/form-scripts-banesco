(function () {
  ...
  function injectCSS() {
    if (document.getElementById("banesco-pdv-v14-safe-css")) return;

    var style = document.createElement("style");
    style.id = "banesco-pdv-v14-safe-css";
    ...
    style.innerHTML = `
      ...
      /* Contenedor de campos sin destruir ExtJS */
      .banesco-grid-safe {
      ...
      /*
        MUY IMPORTANTE:
        No se fuerza display/position/visibility de wrappers internos.
        Solo se limpian colores, borde y fondo.
      */
  ...
  function getOrder(block) {
    ...
    if (txt.indexOf("persona") !== -1 || txt.indexOf("contacto") !== -1) return 20;
    if (txt.indexOf("nacionalidad") !== -1) return 30;
    if (txt.indexOf("documento") !== -1 || txt.indexOf("rif") !== -1 || txt.indexOf("identidad") !== -1) return 40;
    if (txt.indexOf("actividad") !== -1) return 50;
    if (txt.indexOf("teléfono") !== -1 || txt.indexOf("telefono") !== -1 || txt.indexOf("celular") !== -1) return 60;
    if (txt.indexOf("correo") !== -1 || txt.indexOf("email") !== -1) return 70;
    if (txt.indexOf("estado") !== -1) return 80;
    if (txt.indexOf("selecciona la agencia") !== -1) return 90;
    if (txt.indexOf("agencia") !== -1) return 95;
  ...
  function apply() {
    ...
    if (ok) {
      console.log("Banesco PDV v14 safe layout aplicado correctamente.");
    }
